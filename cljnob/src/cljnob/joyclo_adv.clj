...
;; Tail call optimization is a broader term that not only referes to the recursive case
;; (inface recursive tail call optimization is a special case).
;; JVM doesn't have TCO
(defn gcd [x y]
  (cond
    (= x 0) y
    (= x 0) x
...
;; recur can't work on the mutual recursion, as it is, and we can land into the same trouble
;; of so
(defn my-evn? [n]
  (letfn [(evn? [n]
            (if (= n 0)
...
;; Finite State Automata and language processing are few example where mutual recurssion makes sense
...
(fac 20000N)
;; compare the above with normal recursive factorial
(defn fact [n]
  (letfn [(fct [v acc]
            (if (zero? v)
              acc
              (recur (dec v) (* v acc))))]
    (fct n 1)))
;; this won't get into so, tail optimized
(fact 200000N)
...
;; This is amazing!!!!
;; Generalizing the CPS, a powerful approach to construct algorithm from the function
;; A factory of fucntions
(defn mk-cps [accept? kend kont]
  (fn [n]      ;; This is just a wrapper function to hold the computation done in below func
    ((fn [n k] ;; This does the computations
       (let [cont (fn [v]
                    (k ((partial kont v) n)))]
         (if (accept? n)
           (k 1)
...
;; We can create functions based on this mk-cps
(def fac (mk-cps zero? identity #(* %1 %2)))
(def triangle (mk-cps #(== 1 %)
                      identity
                      #(+ %1 %2)))
(fac 30N)
(triangle 4)
;; In the mk-cps also note the use of closures. In fact it returns
;; a closure and the computational nested function is also a closure

;; But CPS is unfortunately not much prevelant in clojure, becuase
;; 1. no tail call optimization in clojure
;; 2. difficult bug tracking
;; 3. doesn't play well with parallel algorithms

;; 
;; fac 2
;;   -> fac-cps 2 identity
;;      -> fac-cps 1 [k (fn [v] (identity (* v 2)))]
;;         -> fac-cps 0 (k 1) -> calls the previous function with the value 1
;;         -> y

;;  1. cont 1 -> (identity (* v 3))
;;  2. cont 2 -> (cont1 (* v 2))
;;  3. cont 3 -> (cont2 (* v 1))
;;  4. cont 4 -> (cont3 (* v 0))
...
(assoc-in [[1 nil]] [0 1] {:a 3})
(sorted-set [1 [3 4]] [1 [3 4]])
(sorted-set 1 8 3)
...
[{:cost 17, :yxs [[0 0] [0 1] [0 2] [0 3] [0 4]
                                          [1 4] 
                  [2 0] [2 1] [2 2] [2 3] [2 4] 
                  [3 0] 
                  [4 0] [4 1] [4 2] [4 3] [4 4]]}
...
v          ;; evaluate to 4
(eval v)   ;; v evaluate to 4 and it becomes (eval 4) which evaluates to 4

'v          ;; evaluates to symbol v
(eval 'v)   ;; evaluates to 4

`v          ;; evaluates to the symbol v, with namespace info
(eval `v)   ;; evaluates to 4

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; unquoting

'~v         ;; evaluates to (clojure.core/unquote v), takes ~v as it is due to the external '
(eval '~v)  ;; attempting to call unbound fn unquote exceptioin

`~v         ;; evaluates to 4, ` and ~ cancels
(eval `~v)  ;; evaluates to 4, much like the first case

`'~v  ;;(quote 4)
`~'v
'`~v ;;v and is equivalent to 'v
...
;; Macro examples
;; Macro defining control structure without syntax-quoting
...
;; corresponding function to the above macro
(defn do-untilfn [& clauses]
  (when clauses
    (if (first clauses)
      (if (next clauses)
        (second clauses)
...
;; defining macros using quoting and unquoting
(defmacro unless [cond & body]
  `(when (not ~cond)
     ~@body))
;; Book implemented it as
(defmacro unless-book [cond & body]
  `(if (not ~cond)
     (do ~@body)))
;; I found out that when uses if/do 

(unless false (inc 3) (dec 4))
(unless true "no way")

(macroexpand-1 '(unless false (inc 3) (dec 4))) ;; don't expand the form if it's a macro
(macroexpand '(unless false (inc 3) (dec 4)))   ;; expand every form, even if it's a macro

;; Pitfalls with macro:
;; 1. forgetting to add ~, will make clj search the namespace for var, here cond
;;     
...
;; Note above that value is a list and doing ~value while defining name would try to evaluate that
;; if passed 20, it will try to (20) which is not a function.
;; so ~@ gives us a way to not evaluate the list but evaluate the elements of a list
...
`(0 55 99 ~@'(1 4 5)) ;; NOTE: ' needed here because 
...
;; lets write binary search in clojure!!!!
  ...
  (letfn [(binhelper
            ...
            (if (< low high)
              (let [mid (quot (+ low high) 2)
                ...
                (cond
                  (== s mval) mid
                  (> s mval) (recur (inc mid) high) 
                  (< s mval) (recur low (dec mid))))))]
    (binhelper 0 (count inp))))

(binsearch [1 4 5 6 9] 9)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;      IDIOMS      ;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;; Knitty-gritties
;; 1. & (rest) argument to function is a ArraySeq, and it doesn't implement IFn 
      ([1 4 9] 0) ;; -> gives the 4
      ;; but below
      (defn testcall [_ & res]
      (res 2))

      (testcall 2 4 5 8) ;; blows up because ArraySeq doesn't implement the IFn

;; 2. A point in addition to 1 is, PersistentVector doesn't implement ISeq; it is seqable, but not a seq
;; 3. PersistentList doesn't implement the IFn
